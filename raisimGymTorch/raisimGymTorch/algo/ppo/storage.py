...
class RolloutStorage:
    def __init__(
        ...
        self.critic_obs = np.zeros(
            [num_transitions_per_env, num_envs, *critic_obs_shape], dtype=np.float32
        )
        self.actor_obs = np.zeros(
            [num_transitions_per_env, num_envs, *actor_obs_shape], dtype=np.float32
        )
        self.rewards = np.zeros(
            [num_transitions_per_env, num_envs, 1], dtype=np.float32
        )
        self.actions = np.zeros(
            [num_transitions_per_env, num_envs, *actions_shape], dtype=np.float32
        )
        self.dones = np.zeros([num_transitions_per_env, num_envs, 1], dtype=bool)

        # For PPO
        self.actions_log_prob = np.zeros(
            [num_transitions_per_env, num_envs, 1], dtype=np.float32
        )
        ...
        self.actions_log_prob_tc = torch.from_numpy(self.actions_log_prob).to(
            self.device
        )
    ...
    def compute_returns(self, last_values, critic, gamma, lam):
        with torch.no_grad():
            self.values = (
                critic.predict(torch.from_numpy(self.critic_obs).to(self.device))
                .cpu()
                .numpy()
            )
    ...
    def mini_batch_generator_shuffle(self, num_mini_batches):
        ...
        for indices in BatchSampler(
            ...
            actor_obs_batch = self.actor_obs_tc.view(-1, *self.actor_obs_tc.size()[2:])[
                indices
            ]
            critic_obs_batch = self.critic_obs_tc.view(
                -1, *self.critic_obs_tc.size()[2:]
            )[indices]
    ...
    def mini_batch_generator_inorder(self, num_mini_batches):
        ...
        for batch_id in range(num_mini_batches):
            yield (
                self.actor_obs_tc.view(-1, *self.actor_obs_tc.size()[2:])[
                    batch_id * mini_batch_size : (batch_id + 1) * mini_batch_size
                ],
                self.critic_obs_tc.view(-1, *self.critic_obs_tc.size()[2:])[
                    batch_id * mini_batch_size : (batch_id + 1) * mini_batch_size
                ],
                self.actions_tc.view(-1, self.actions_tc.size(-1))[
                    batch_id * mini_batch_size : (batch_id + 1) * mini_batch_size
                ],
                self.sigma_tc.view(-1, self.sigma_tc.size(-1))[
                    batch_id * mini_batch_size : (batch_id + 1) * mini_batch_size
                ],
                self.mu_tc.view(-1, self.mu_tc.size(-1))[
                    batch_id * mini_batch_size : (batch_id + 1) * mini_batch_size
                ],
                self.values_tc.view(-1, 1)[
                    batch_id * mini_batch_size : (batch_id + 1) * mini_batch_size
                ],
                self.advantages_tc.view(-1, 1)[
                    batch_id * mini_batch_size : (batch_id + 1) * mini_batch_size
                ],
                self.returns_tc.view(-1, 1)[
                    batch_id * mini_batch_size : (batch_id + 1) * mini_batch_size
                ],
                self.actions_log_prob_tc.view(-1, 1)[
                    batch_id * mini_batch_size : (batch_id + 1) * mini_batch_size
                ],
            )
