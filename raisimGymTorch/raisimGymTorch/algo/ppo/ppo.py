...
class PPO:
    ...
    def act(self, actor_obs):
        ...
        with torch.no_grad():
            self.actions, self.actions_log_prob = self.actor.sample(
                torch.from_numpy(actor_obs).to(self.device)
            )
        return self.actions
    ...
    def step(self, value_obs, rews, dones):
        self.storage.add_transitions(
            ...
            self.actor.distribution.std_np,
            rews,
            dones,
    ...
    def update(self, actor_obs, value_obs, log_this_iteration, update):
        ...
        self.storage.compute_returns(
            last_values.to(self.device), self.critic, self.gamma, self.lam
        )
    ...
    def log(self, variables):
        ...
        self.writer.add_scalar(
            "PPO/value_function", variables["mean_value_loss"], variables["it"]
        )
        self.writer.add_scalar(
            "PPO/surrogate", variables["mean_surrogate_loss"], variables["it"]
        )
