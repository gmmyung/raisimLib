...
class RewardAnalyzer:
    def __init__(self, env, writer):
        ...
        self.data_min = np.inf * np.ones(
            shape=(len(self.data_tags), 1), dtype=np.double
        )
        self.data_max = -np.inf * np.ones(
            shape=(len(self.data_tags), 1), dtype=np.double
        )
    ...
    def add_reward_info(self, info):
        ...
        for i in range(len(self.data_tags)):
            for j in range(len(info)):
                self.data_square_sum[i] += (
                    info[j][self.data_tags[i]] * info[j][self.data_tags[i]]
                )
    ...
    def analyze_and_plot(self, step):
        ...
        data_std = np.sqrt(
            (self.data_square_sum - self.data_size * self.data_mean * self.data_mean)
            / (self.data_size - 1 + 1e-16)
        )

        for data_id in range(len(self.data_tags)):
            self.writer.add_scalar(
                self.data_tags[data_id] + "/mean",
                self.data_mean[data_id],
                global_step=step,
            )
            self.writer.add_scalar(
                self.data_tags[data_id] + "/std", data_std[data_id], global_step=step
            )
            self.writer.add_scalar(
                self.data_tags[data_id] + "/min",
                self.data_min[data_id],
                global_step=step,
            )
            self.writer.add_scalar(
                self.data_tags[data_id] + "/max",
                self.data_max[data_id],
                global_step=step,
            )
