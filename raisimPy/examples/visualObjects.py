...
# articulated system
laikago = server.addVisualArticulatedSystem(
    "v_laikago", rscDir + "laikago/laikago.urdf"
)
laikago.setGeneralizedCoordinate(
    np.array(
        [
            0,
            0,
...
# visualmesh
vertex = np.array(
    [0.5, 0.5, 0.0, 0.0, -1.0, 0.0, -0.5, 0.5, 0.0, 0.0, 0.0, 1.0], dtype=np.float32
)
...
for i in range(100000):
    counter = counter + 1
    visBox.setColor(1, 1, (counter % 255 + 1) / 256.0, 1)
    visSphere.setColor(1, (counter % 255 + 1) / 256.0, 1, 1)
    lines.setColor(1 - (counter % 255 + 1) / 256.0, 1, (counter % 255 + 1) / 256.0, 1)
    visBox.setBoxSize((counter % 255 + 1) / 256.0 + 0.01, 1, 1)
    vertex[0] = 0.5 * (2.0 + math.sin(counter * 0.01))
